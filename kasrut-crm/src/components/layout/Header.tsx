    ...
    <>
      <header className="app-header">
        ...
        <div className="header-right">
          ...
          <button
            title={t.twoFactor?.settingsTitle ?? '2FA'}
            onClick={() => setShow2faPanel(v => !v)}
            className={show2faPanel ? 'btn-ghost btn-ghost--active' : 'btn-ghost'}
            style={user?.twoFactorEnabled ? { color: 'var(--status-ok)' } : undefined}
          >
            🔐
          </button>
      ...
      {show2faPanel && (
        <>
          <div className="tf-overlay" onClick={() => setShow2faPanel(false)} />
